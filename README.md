...
```
  ...
  Query                                                                                       │                 │
   ...
   ▼                                                                                          │                 │
┌────────────┐    ┌────────┐    ┌───────────┐    ┌─────────┐    ┌──────────────┐              │                 │
│ Classifier │───▶│ Router │───▶│ Validator │───▶│ Planner │───▶│  WebAgent_1  │──┐           │                 │
│   (MoE)    │    │        │    │ (skip for │    │ + memory│    │ search+read  │  │           │                 │
└────────────┘    └────────┘    │  simple)  │    │ context │    └──────────────┘  │           │                 │
                                └───────────┘    └─────────┘    ┌──────────────┐  │   ┌──────────────┐    ┌─────────────┐    ┌────────────┐    ┌────────┐
                                                                │  WebAgent_2  │──┼──▶│ Synthesizer  │───▶│  Distiller  │───▶│  Critic +  │───▶│ Report │
                                                                │ search+read  │  │   │  (evidence   │    │  (skip for  │    │ Fact-Check │    └────────┘
                                                                └──────────────┘  │   │   chains)    │    │   simple)   │    └────────────┘
                                                                      ...         │   └──────────────┘    └─────────────┘
                                                                ┌──────────────┐  │
                                                                │  WebAgent_N  │──┘
  ...
  Each WebAgent has autonomous access to:  web_search · search_news · quick_answer · fetch_url · crawl_links · academic_search
  Runs in parallel — the DAG executor (graphlib) waits for all to finish before Synthesizer.
```
